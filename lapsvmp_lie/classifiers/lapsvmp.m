...
while 1  
    ...
    if options.Hinge && ~options.roboss       
        sv_prev=sv;         
        hloss=sparse([],[],[],n,1,l);
        hloss(labeled)=1-data.Y(labeled).*(Kalpha(labeled,:)+b);    
        sv=hloss>0;
        nsv=nnz(sv);
    elseif options.Hinge && options.roboss 
        options.a=1.4;
        options.lambda=1;
        sv_prev = sv;
        u=sparse([],[],[],n,1,l);
        u(labeled)=1-data.Y(labeled).*(Kalpha(labeled,:)+b); 
        hloss = roboSS_loss(u, options.a, options.lambda);
        sv = hloss > 0;
        nsv = nnz(sv);
...
%                 lemda=1e-5;
%                 alpha_new=(lemda*(gamma_A*speye(n)+IsvK+gamma_I*LK))\IsvY;
